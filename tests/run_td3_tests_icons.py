...
# Symboles Unicode pour le rapport (comme dans TD2)
SYMBOL_OK = "✅"
SYMBOL_FAIL = "❌"
SYMBOL_WARNING = "⚠️"
...
def analyze_td3_files():
    ...
    with open('analysis_results_td3_with_icons.txt', 'w', encoding='utf-8') as out_file:
        ...
        for exercise in td3_config['exercises']:
            ...
            for file in java_files:
                ...
                patterns_status = SYMBOL_OK if not missing_patterns else SYMBOL_WARNING
                msg = f"\n{patterns_status} PATTERNS REQUIS:"
                print(msg)
                out_file.write(msg + '\n')
                ...
                for pattern_info in custom_patterns:
                    ...
                    if is_missing and error_msg:
                        msg = f"      {SYMBOL_WARNING} {error_msg}"
                        print(msg)
                        out_file.write(msg + '\n')
                ...
                # Note estimée
                max_points = next((ex['maxPoints'] for ex in td3_config['exercises'] if ex['exerciseId'] == exercise_id), 10)
                ...
                msg = f"NOTE ESTIMÉE: {estimated_score}/{max_points} points"
                print(msg)
                out_file.write(msg + '\n')
                ...
                if not all_ok:
                    ...
                    if not methods_ok:
                        msg = f"    {SYMBOL_FAIL} Méthodes manquantes ou incorrectes"
                        print(msg)
                        out_file.write(msg + '\n')
                    ...
                    if not control_structures_ok:
                        msg = f"    {SYMBOL_FAIL} Structures de contrôle manquantes"
                        print(msg)
                        out_file.write(msg + '\n')
                    
                    if not naming_ok:
                        msg = f"    {SYMBOL_FAIL} Conventions de nommage non respectées"
                        print(msg)
                        out_file.write(msg + '\n')
                    
                    if not scope_ok:
                        msg = f"    {SYMBOL_FAIL} Problèmes de portée des variables"
                        print(msg)
                        out_file.write(msg + '\n')
                else:
